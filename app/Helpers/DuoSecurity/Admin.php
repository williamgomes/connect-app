...
class Admin extends Client
{
    ...
    public function enroll_user($username, $email, $validSecs = null)
    {
        assert(is_string($username));
        assert(is_string($email));
        assert(is_string($validSecs) || is_null($validSecs));

        $method = 'POST';
        $endpoint = '/admin/v1/users/enroll';
        $params = [
            'username' => $username,
            'email'    => $email,
        ];
    ...
    public function delete_user($userId)
    {
        ...
        $method = 'DELETE';
        $endpoint = '/admin/v1/users/' . $userId;
        $params = [];
