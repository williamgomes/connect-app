    ...
    <nav class="flex items-center justify-between text-black bg-navbar shadow-xs h-16">
        <div class="flex items-center flex-no-shrink">
            <a href="{{ url('/') }}" class="flex items-center flex-no-shrink text-black mx-4">
                <img style="height: 30px;" src="{{ asset('web/images//logo/synega-connect.png') }}" alt="{{ env('COMPANY_NAME' ) . ' ' . __('Logo') }}">
            </a>
                    ...
                    <ul class="list-reset">
                        @foreach ($versions as $version)
                            <li class="py-2 hover:bg-grey-lightest">
                                <a class="px-6 text-grey-darkest" href="{{ route('larecipe.show', ['version' => $version, 'page' => 'overview']) }}">{{ $version }}</a>
                            </li>
                        @endforeach
                    </ul>
                </template>
